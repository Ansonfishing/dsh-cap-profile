...
const TOP_N = 5;
const TOOLS_TOP_N = 10; // P2.0:对比矩阵用每模型 Top-10 工具(长尾截断,体积+~5.6KB)
  ...
  getPayload(range) {
    ...
    if (winLabel !== 0) {
      ...
      for (const [id, m] of this.live.models.entries()) {
        ...
        // P2.0:对比矩阵工具源(Top-10,窗口口径)+ 逐日 series(只含窗口内日)
        const toolsTop = [...tools.entries()]
          .sort((a, b) => b[1].calls - a[1].calls || a[0].localeCompare(b[0]))
          .slice(0, TOOLS_TOP_N)
          .map(([tool, x]) => ({ tool, calls: x.calls, errors: x.errors }));
        const series = [...m.days.entries()]
          .filter(([dk]) => dk >= from && dk <= to)
          .sort((a, b) => (a[0] < b[0] ? -1 : 1))
          .map(([d, de]) => ({ d, calls: de.calls, errors: de.errors }));
        models.push({
          id,
          sessions,
          toolCalls,
    ...
    const models = [...this.live.models.entries()].map(([id, m]) => {
      ...
      const topErrors = [...m.errors.entries()]
        ...
        .slice(0, TOP_N)
        .map(([signature, count]) => ({ signature, count }));
      // P2.0:对比矩阵工具源(Top-10,全量口径)+ 逐日 series(全部活跃日升序)
      const toolsTop = [...m.tools.entries()]
        .sort((a, b) => b[1].calls - a[1].calls || a[0].localeCompare(b[0]))
        .slice(0, TOOLS_TOP_N)
        .map(([tool, x]) => ({ tool, calls: x.calls, errors: x.errors }));
      const series = [...m.days.entries()]
